...
class Zipkin(object):
    ...
    def _before_request(self):
        ...
        if safe_headers.get('traceparent'):
            elems = safe_headers.get('traceparent').split('-')
            trace_id = elems[1]
            parent_span_id = elems[2]

        flags = safe_headers.get('x-b3-flags') or safe_headers.get('traceparent')
